...
!===============================================================================
  ...
  subroutine pmxsub(lchnk, ncol, pint, pmid, coszrs, state, t, cld, qm1, Nnatk, &
    ...
    real(r8) :: f_soa(pcols,pver),f_soana(pcols,pver), vnbc, vaitbc
    ...
    real(r8) :: ck(pcols,pver,0:nmodes)
    ...
    real(r8) :: dod6703d(pcols,pver), abs6703d(pcols,pver)
    ...
    real(r8) :: dod8703d(pcols,pver), abs8703d(pcols,pver)
    ...
    real(r8) :: dod8703d_pom(pcols,pver) ! abs8703d_pom(pcols,pver)
    real(r8) :: dod5503dlt1_ss(pcols,pver), dod5503dgt1_ss(pcols,pver)
    real(r8) :: dod5503dlt1_dust(pcols,pver), dod5503dgt1_dust(pcols,pver)
    real(r8) :: dod5503dlt1_so4(pcols,pver), dod5503dgt1_so4(pcols,pver)
    real(r8) :: dod5503dlt1_bc(pcols,pver), dod5503dgt1_bc(pcols,pver)
    real(r8) :: dod5503dlt1_pom(pcols,pver), dod5503dgt1_pom(pcols,pver)
    real(r8) :: dod440(pcols), abs440(pcols), dod500(pcols), abs500(pcols)
    real(r8) :: dod550(pcols), abs550(pcols), abs550alt(pcols), dod670(pcols)
    real(r8) :: abs670(pcols), dod870(pcols), abs870(pcols)
    ...
    real(r8) :: dod550lt1_ss(pcols), dod550gt1_ss(pcols), dod550lt1_dust(pcols)
    real(r8) :: dod550gt1_dust(pcols), dod550lt1_so4(pcols)
    real(r8) :: dod550gt1_so4(pcols), dod550lt1_bc(pcols), dod550gt1_bc(pcols)
    real(r8) :: dod550lt1_pom(pcols), dod550gt1_pom(pcols)
    real(r8) :: abs550_ss(pcols), abs550_dust(pcols)
    real(r8) :: abs550_so4(pcols), abs550_bc(pcols), abs550_pom(pcols)
    ...
    ra#ifdef AEROCOM  ! AEROCOM***********AEROCOM**************AEROCOM***************below   
    ...
    do i=0,nbmodes
       do k=1,pver
          do icol=1,ncol
             ...
             if(i>=1) then
                bebc440tot(icol,k)=bebc440tot(icol,k)+Nnatk(icol,k,i) * extinction_coeff%bebc440(icol,k,i)
                bebc500tot(icol,k)=bebc500tot(icol,k)+Nnatk(icol,k,i) * extinction_coeff%bebc500(icol,k,i)
                bebc550tot(icol,k)=bebc550tot(icol,k)+Nnatk(icol,k,i) * extinction_coeff%bebc550(icol,k,i)
                babc550tot(icol,k)=babc550tot(icol,k)+Nnatk(icol,k,i) * extinction_coeff%babc550(icol,k,i)
                bebc670tot(icol,k)=bebc670tot(icol,k)+Nnatk(icol,k,i) * extinction_coeff%bebc670(icol,k,i)
                bebc870tot(icol,k)=bebc870tot(icol,k)+Nnatk(icol,k,i) * extinction_coeff%bebc870(icol,k,i)
                beoc440tot(icol,k)=beoc440tot(icol,k)+Nnatk(icol,k,i) * extinction_coeff%beoc440(icol,k,i)
                beoc500tot(icol,k)=beoc500tot(icol,k)+Nnatk(icol,k,i) * extinction_coeff%beoc500(icol,k,i)
                beoc550tot(icol,k)=beoc550tot(icol,k)+Nnatk(icol,k,i) * extinction_coeff%beoc550(icol,k,i)
                baoc550tot(icol,k)=baoc550tot(icol,k)+Nnatk(icol,k,i) * extinction_coeff%baoc550(icol,k,i)
                beoc670tot(icol,k)=beoc670tot(icol,k)+Nnatk(icol,k,i) * extinction_coeff%beoc670(icol,k,i)
                beoc870tot(icol,k)=beoc870tot(icol,k)+Nnatk(icol,k,i) * extinction_coeff%beoc870(icol,k,i)
             endif  ! i>=1
             if(i==6.or.i==7) then
    ...
    ! BC(ax) mode
    aerodry_prop%intdrypar0(lchnk, ncol, Nnatk)

    ! SO4&SOA(Ait,n) mode
    aerodry_prop%intdrypar1(lchnk, ncol, Nnatk, xfombg, ifombg1, xct, ict1, xfac, ifac1)

    ! BC(Ait,n) and OC(Ait,n) modes
    aerodry_prop%intdrypar2to3(lchnk, ncol, Nnatk, xct, ict1, xfac, ifac1)

    ! BC&OC(Ait,n) mode   ------ fcm not valid here (=0). Use faitbc or fnbc instead
    aerodry_prop%intdrypar4(lchnk, ncol, Nnatk, &
         xfbcbg, ifbcbg1, xfbcbgn, ifbcbgn1, &
         xct, ict1, xfac, ifac1, xfaq, ifaq1)

    ! SO4(Ait75) (5), mineral (6-7) and Sea-salt (8-10) modes:
    aerodry_prop%intdrypar5to10(lchnk, ncol, Nnatk, &
    ...
    do k=1,pver
       do icol=1,ncol
          ...
          c_mi(icol,k)    = Nnatk(icol,k,6) * aerdry_prop%cintbg(icol,k,6) + &
               Nnatk(icol,k,7) * aerdry_prop%cintbg(icol,k,7)
          c_mi05(icol,k)  = Nnatk(icol,k,6) * aerdry_prop%cintbg05(icol,k,6) +  &
               Nnatk(icol,k,7) * aerdry_prop%cintbg05(icol,k,7)
          c_mi125(icol,k) = Nnatk(icol,k,6) * aerdry_prop%cintbg125(icol,k,6) +  &
               Nnatk(icol,k,7) * aerdry_prop%cintbg125(icol,k,7) 
          c_ss(icol,k)    = Nnatk(icol,k,8) * aerdry_prop%cintbg(icol,k,8) + &
               Nnatk(icol,k,9) * aerdry_prop%cintbg(icol,k,9) + &
               Nnatk(icol,k,10)* aerdry_prop%cintbg(icol,k,10)
          c_ss05(icol,k)  = Nnatk(icol,k,8) * aerdry_prop%cintbg05(icol,k,8) + &
               Nnatk(icol,k,9) * aerdry_prop%cintbg05(icol,k,9) + &
               Nnatk(icol,k,10)* aerdry_prop%cintbg05(icol,k,10)
          c_ss125(icol,k) = Nnatk(icol,k,8) * aerdry_prop%cintbg125(icol,k,8) + &
               Nnatk(icol,k,9) * aerdry_prop%cintbg125(icol,k,9) + &
               Nnatk(icol,k,10)* aerdry_prop%cintbg125(icol,k,10)
